...
experiment_root="$runtime_root/logs/MSIO-CP-E009R1"
raw_root="$experiment_root/raw"
lock_root="$runtime_root/locks/MSIO-CP-E009R1.lock"
...
if ! mkdir "$lock_root" 2>/dev/null; then echo E009R1_LOCKED; exit 75; fi
...
while read -r block position mode; do
  ...
  for settle_index in 1 2; do
    if test -z "$(pgrep -x llama-cli || true)"; then break; fi
    sleep 1
...
if test "$analysis_status" = 0; then echo E009R1_PASS; else echo E009R1_NO_GO; exit "$analysis_status"; fi
