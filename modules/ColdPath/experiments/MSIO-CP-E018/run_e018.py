...
OUT=ROOT/'logs/MSIO-CP-E018'; RAW=OUT/'raw'
SCHEDULE=[('b1','cold'),('b1','prefetch'),('b2','prefetch'),('b2','cold'),('b3','cold'),('b3','prefetch'),('b4','prefetch'),('b4','cold'),('b5','cold'),('b5','prefetch'),('b6','prefetch'),('b6','cold')]
...
def residency(fd,n):
    ps=os.sysconf('SC_PAGE_SIZE'); pages=(n+ps-1)//ps
...
def main():
    ...
    OUT.mkdir(parents=True,exist_ok=False); RAW.mkdir()
    rows=[]
    for block,arm in SCHEDULE:
        ...
        with open(MODEL,'rb',buffering=0) as f:
            ...
            cold=residency(f.fileno(),MODEL.stat().st_size)
            if cold>0.20: return 93
            prep=0.0; ready=cold
            if arm=='prefetch':
                started=time.monotonic()
                while f.read(8<<20): pass
                prep=time.monotonic()-started; ready=residency(f.fileno(),MODEL.stat().st_size)
                if ready<0.80: return 94
        ...
        receipt.update({'experiment':'MSIO-CP-E018','block':block,'arm':arm,'cold_fraction':cold,'ready_fraction':ready,'prefetch_s':prep})
        (RAW/f'{trial}.e018.json').write_text(json.dumps(receipt,sort_keys=True)+'\n')
        rows.append(receipt)
