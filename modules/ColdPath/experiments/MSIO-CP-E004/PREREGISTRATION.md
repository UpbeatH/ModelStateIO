...
- Acquisition outcome: the official artifact was downloaded and SHA-256-verified on Windows, uploaded by the user to the fixed g130 top-level inbox, then reverified before and after staging at `/mnt/nvme1/chenhao/modelstateio-runtime/incoming/qwen2.5-0.5b-instruct-q4_k_m.gguf`. E004 must use this staged artifact and must not perform any remote download.
...
- Remote download was attempted once but stalled; its process and a zero-byte partial file were preserved as acquisition-failure evidence. The verified staged artifact above replaces that path; do not start a parallel or replacement remote transfer.
