...
experiment_root="$runtime_root/logs/MSIO-CP-E005"
lock_root="$runtime_root/locks/MSIO-CP-E005.lock"
...
(
  ulimit -f 2048
  LD_LIBRARY_PATH=/usr/local/cuda-11.6/lib64 timeout 120 "$binary_file" --model "$model_file" --load-mode mmap --single-turn --simple-io --no-display-prompt --prompt 'Reply with exactly OK.' --predict 1 --seed 1 --temp 0 --n-gpu-layers 99 --ctx-size 256 --batch-size 64 --threads 4 --threads-batch 4
) > "$experiment_root/stdout.txt" 2> "$experiment_root/stderr.txt"
smoke_status=$?
set -e
...
test "$smoke_status" = 0
test "$(stat -c %s "$experiment_root/stdout.txt")" -le 1048576
test "$(stat -c %s "$experiment_root/stderr.txt")" -le 1048576
test -z "$(pgrep -u "$(id -u)" -f '[l]lama-cli' || true)"
sha256sum "$experiment_root"/*.txt "$experiment_root"/*.exit > "$experiment_root/SHA256SUMS.txt"
