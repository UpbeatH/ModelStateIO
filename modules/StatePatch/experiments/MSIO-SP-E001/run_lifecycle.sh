#!/usr/bin/env bash
# Bounded E001 runner.  All mutable output stays outside the Git working tree.
set -euo pipefail
...
adapter="$runtime/artifacts/statepatch-e001/qwen2.5-0.5b-gsm8k-lora.gguf"
out="$runtime/logs/MSIO-SP-E001/$(date +%Y%m%dT%H%M%S%z)"
port=18097
mkdir -p "$out"
...
for _ in $(seq 1 45); do
  if curl --silent --show-error --fail --max-time 2 "http://127.0.0.1:$port/lora-adapters" > "$out/adapters.initial.json"; then
    break
  fi
  sleep 1
done
...
if not isinstance(value, str):
    raise SystemExit('response lacks string content')
print(value, end='')
PY
}

curl --silent --show-error --fail --max-time 10 -H 'Content-Type: application/json' \
  -d '[{"id":0,"scale":0.0}]' "http://127.0.0.1:$port/lora-adapters" > "$out/control.disabled-1.json"
request disabled_1
curl --silent --show-error --fail --max-time 10 -H 'Content-Type: application/json' \
  -d '[{"id":0,"scale":1.0}]' "http://127.0.0.1:$port/lora-adapters" > "$out/control.enabled.json"
request enabled
curl --silent --show-error --fail --max-time 10 -H 'Content-Type: application/json' \
  -d '[]' "http://127.0.0.1:$port/lora-adapters" > "$out/control.disabled-2.json"
request disabled_2
...
def content(name): return (root / (name + '.content')).read_text(encoding='utf-8')
def h(s): return hashlib.sha256(s.encode()).hexdigest()
a, b, c = map(content, ('disabled_1', 'enabled', 'disabled_2'))
result = {
  'disabled_equal': a == c,
  'enabled_differs': a != b,
  'content_sha256': {'disabled_1': h(a), 'enabled': h(b), 'disabled_2': h(c)},
  'decision': 'GO (technical)' if a == c and a != b else 'NO-GO',
}
